...
class RotaryTransformerEncoderCTCModule(pl.LightningModule):
    NUM_BANDS: ClassVar[int] = 2
    ELECTRODE_CHANNELS: ClassVar[int] = 16
    ...
    def __init__(
        ...
        # Compute the number of features after the MLP
        num_features = self.NUM_BANDS * mlp_features[-1]

        # Model
        # Expected input: (T, N, bands=2, electrode_channels=16, freq)
        self.model = nn.Sequential(
            # Normalize the spectrogram input.
            SpectrogramNorm(channels=self.NUM_BANDS * self.ELECTRODE_CHANNELS),
            # Multi-band MLP which outputs: (T, N, bands=2, mlp_features[-1])
            MultiBandRotationInvariantMLP(
                in_features=in_features,
                mlp_features=mlp_features,
                num_bands=self.NUM_BANDS,
            ),
            # Flatten to shape: (T, N, num_features)
            nn.Flatten(start_dim=2),
            # Swap in the Rotary Transformer Encoder
            RotaryTransformerEncoder(
                num_features=num_features,
                num_layers=num_layers,
                d_model=d_model,
            ...
            # Unembedding layer: maps Transformer d_model contextual vectors to class logits per time step.
            nn.Linear(d_model, charset().num_classes),
            nn.LogSoftmax(dim=-1),
        )
        ...
        # Metrics for train, validation, and test phases
        metrics = MetricCollection([CharacterErrorRates()])
        self.metrics = nn.ModuleDict(
            {
                f"{phase}_metrics": metrics.clone(prefix=f"{phase}/")
    ...
    def _step(
        ...
        # Adjust input lengths by the temporal difference introduced by the encoder
        T_diff = inputs.shape[0] - emissions.shape[0]
        emission_lengths = input_lengths - T_diff
        ...
        # Update metrics per batch entry
        metrics = self.metrics[f"{phase}_metrics"]
